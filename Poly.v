...
(** **** Exercise: 1 star, standard, optional (combine_checks)
    ...
    - What is the type of [combine] (i.e., what does [Check
      @combine] print?)
    answer: 
    forall X, Y: Type, X -> Y -> list X -> list Y -> list(X*Y)
    - What does
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n:nat => andb (even n) (ltb 7 n)) l.
...
(** **** Exercise: 3 stars, standard (map_rev)
    ...
    auxiliary lemma. *)
Lemma single_map: forall (X Y: Type) (f: X -> Y) (h : X),
  map f [h] = [f h].
Proof. simpl. reflexivity. Qed.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'. simpl. rewrite <- single_map. rewrite -> map_distr. reflexivity.
Qed.
(** [] *)
...
(* There is a situation when X is int and Y is a boolean and we can apply
  the fold function to determine whether there is a number which is greater
...
*)
(* 
  if l equals [], then @nth_error X l n = None.

  when l equals a :: l' and for l', we have IHl':
    length l' = n' -> @nth_error X l' n' = None.
  
  H: length l = length a :: l' = S (length l') = S n'.
  implies that length l
